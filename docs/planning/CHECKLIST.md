...
| R14 | **Open — diagnosed, not fixed.** The pedestal's procedural texture (F16) produces no pixel change at all: five patterns at full strength render a byte-identical frame (`scripts/out/texture-pedestal-*.png` are the same file five times). Two hypotheses were tested and both were wrong — raising the plinth's contrast against the backdrop from 1.13:1 to 1.4:1 moved the PNG by 17 bytes out of 148KB, and the R3F map-recompile fix (`mapsKey`) did not move it either. What that 17 bytes shows is the actual cause: **the plinth has almost no visible area at the default camera**, hidden behind the device and under the contact shadow, so no surface treatment on it can register. Requirement C3 asks for a pedestal that highlights the product, and this one is not visible enough to. Fixing it means changing the shipped composition — the plinth's size, the shadow's extent, or the default camera — which is a design call rather than a bug fix. `verify-textures.mjs` passes only because 0 <= 0 <= 0 is monotonic, so its pedestal assertion is currently vacuous | TODO |
